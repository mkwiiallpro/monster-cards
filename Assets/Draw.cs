...
public class Draw : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        hueSlider.onValueChanged.AddListener(SetBrushColor); // Add a listener to update the brush color when the slider changes
        hueSlider.onValueChanged.AddListener(offToggles); // Add a listener to update the brush color when the slider changes

        // Add a listener to update the brush size when the slider changes
    ...
    private void offToggles(float temp){
        whiteToggle.isOn = false; // Uncheck the white toggle
    ...
    private void SetBrushColor(float newHue)
    {
        brushColor = Color.HSVToRGB(newHue, 1, 1); // Set the brush color based on the hue slider value
        
        // Uncheck other toggles (optional, if you want radio button behavior)

        if (whiteToggle.isOn && brushColor != whiteColor)
            brushColor = Color.white; // Set the brush color to white if the toggle is checked
        if (blackToggle.isOn && brushColor != blackColor)
            brushColor = Color.black; // Set the brush color to black if the toggle is checkedwhite if the toggle is checked
        hueSliderImage.color = brushColor; // Update the hue slider image color to match the brush color
    }
    ...
    void DrawBrush(int xPix, int yPix) //This function takes a point on the canvas as a parameter and draws a circle with radius brushSize around it
    {
        ...
        brushSize = 4 + (int)brushSizeSlider.value;

        SetBrushColor(hueSlider.value);
